...
const QueueManager: React.FC<QueueManagerProps> = ({ user, queue, onBack }) => {
  ...
  const [settings, setSettings] = useState<QueueSettings>(currentQueue.settings || defaultSettings);

  // Sync settings when currentQueue updates (e.g. from backend)
  useEffect(() => {
      if (currentQueue.settings) {
          setSettings(prev => ({ ...prev, ...currentQueue.settings }));
      }
  ...
  useEffect(() => {
    ...
    const startStreaming = async () => {
        try {
            ...
            // Stream dynamic data (visitors, metrics)
            unsubscribe = queueService.streamQueueData(queue.id, (data) => {
                if (data) {
                    setQueueData(data);
  ...
  const updateCharts = (data: QueueData) => {
  ...
  // ... (Rest of Logic similar to previous, ensuring safe access) ...
  // Dynamic Queue Logic
  useEffect(() => {
  ...
  const calculatePredictedWait = () => {
      ...
      if (servedVisitors.length < 3) {
          return { time: (currentQueue.estimatedWaitTime || 5) * queueData.metrics.waiting, activeStaff: 1 };
      }
      // Simple logic for safety
      return { time: Math.max(1, queueData.metrics.waiting * 5), activeStaff: 1 };
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-6 border-b border-gray-100 mb-6">
        ...
        <div className="flex items-center gap-3">
            ...
            <div className="flex gap-2">
                 <button onClick={handleShareLink} className="p-2.5 bg-white border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 shadow-sm" title="Copy Join Link">
                    <Share2 size={20} />
                </button>
                 <button 
                 ...
                 <button onClick={() => window.open(`${window.location.origin}?view=display&queueId=${queue.id}`, '_blank')} className="p-2.5 bg-white border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 shadow-sm" title="Open Public Display">
                    <Monitor size={20} />
                </button>
            </div>
        </div>
...
// ... DraggableVisitorListItem & VisitorListItem (Unchanged) ...
