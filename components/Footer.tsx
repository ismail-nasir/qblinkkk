...
const Footer: React.FC<FooterProps> = ({ onNavigate }) => {
  ...
  return (
    <footer
      id="contact"
      className="py-12 bg-white border-t border-gray-100 relative z-10"
    >
      <div className="container mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="flex flex-wrap justify-center gap-6 md:gap-8 text-sm font-semibold text-gray-500">
          ...
          {/* Keep legal links */}
          <button
            onClick={handleNavClick(AppView.PRIVACY)}
            className="hover:text-primary-600 transition-colors"
          >
            Privacy
          </button>
          <button
            onClick={handleNavClick(AppView.TERMS)}
            className="hover:text-primary-600 transition-colors"
          >
            Terms
          </button>
        ...
        <p className="text-sm text-gray-400">
          © 2025 Qblink. All rights reserved.
        </p>
      </div>
    </footer>
  );
