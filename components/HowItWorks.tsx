           ...
           <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
             {steps.map((step) => (
               <div key={step.id} className="relative z-10 flex flex-col items-center">
                 <GlassCard className="p-8 w-full max-w-sm flex flex-col items-center text-center h-full min-h-[300px] justify-center" hoverEffect={true}>
                    {/* Icon Squarcle */}
                    <div className={`w-24 h-24 ${step.color} rounded-[28px] flex items-center justify-center mb-8 shadow-inner`}>
                      {step.icon}
                    </div>
                 ...
                 {/* Step Number Badge */}
                 <div className="absolute -bottom-6 bg-gray-900 text-white w-12 h-12 rounded-2xl flex items-center justify-center font-bold border-4 border-white shadow-lg">
                   {step.id}
                 </div>
               </div>
             ))}
